    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/seasons/now?sfw`
        );
      ...
      {animeList.map((anime, index) => (
        ...
        >
          ...
          <div className="relative flex flex-col sm:grid sm:grid-cols-5 sm:grid-rows-5 h-full">
            {/* Image container - full width on mobile, 2 cols on larger screens */}
            ...
            {/* Content container - full width on mobile, 3 cols on larger screens */}
            <div className="p-3 sm:col-span-3 sm:row-span-5 sm:col-start-3 sm:row-start-1 text-white flex flex-col">
              <h3 className="font-bold text-base sm:text-lg md:text-xl mb-2 text-white truncate">{anime.title}</h3>

              <div className="flex justify-between mb-2 text-xs sm:text-sm w-full">
                <p className="flex items-center"> 
                  <Radar color="#F85222" fill="#FFD700" className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> 
                  <span className="text-white">Rank: {anime.rank}</span> 
                </p>
                <p className="flex items-center"> 
                  <Flame color="#F85222" fill="#FFD700" className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> 
                  <span className="text-white">Pop: {anime.popularity}</span>
                </p>
                <p className="flex items-center"> 
                  <Star color="#FFD700" fill="#FFD700" className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> 
